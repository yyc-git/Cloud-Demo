...
let init = (device, swapChainFormat, state) => {
  ...
  let (acceptBoolBufferSize, acceptBoolBuffer) =
    RTXBuffer.AcceptBoolBuffer.unsafeGetBufferData(state);
  let (prevFramePixelIndicesBufferSize, prevFramePixelIndicesBuffer) =
    RTXBuffer.PrevFramePixelIndicesBuffer.unsafeGetBufferData(state);
  ...
  let baseShaderPath = "src/pass/bmfr/preprocess/shaders";
