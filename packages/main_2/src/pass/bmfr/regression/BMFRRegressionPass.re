...
let init = (device, state) => {
  ...
  let (tmpDataBufferSize, tmpDataBuffer) =
    RTXBuffer.Regression.TmpDataBuffer.unsafeGetBufferData(state);
  let (outDataBufferSize, outDataBuffer) =
    RTXBuffer.Regression.OutDataBuffer.unsafeGetBufferData(state);
  let (commonDataBufferData, commonDataBuffer) =
    RTXBuffer.Regression.CommonDataBuffer.unsafeGetBufferData(state);
    ...
    |> Device.createBindGroup({
         ...
         "entries": [|
           ...
           BindGroup.binding(
             ...
             ~offset=0,
             ~size=
               RTXBuffer.Regression.CommonDataBuffer.getBufferSize(
...
let execute = (device, queue, window, state) => {
  Pass.AccumulationPass.canDenoise(state)
    ? {
      ...
      let w = RTXBuffer.Regression.computeHorizentalBlocksCount(window);
      let h = RTXBuffer.Regression.computeVerticalBlocksCount(window);
